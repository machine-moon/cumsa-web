...
export default async function LoginPage() {
  ...
  const session = await getIronSession<{ authorized?: boolean }>(cookieStore, draftSessionOptions);
  if (session.authorized) {
    redirect("/extras/portals/drafts/menu");
  }
    ...
    <div className="min-h-[60vh] flex items-center justify-center animate-fade-in">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-extrabold text-[var(--blue)] mb-2">Drafts Portal</h1>
          <p className="text-[var(--navy)]/80">Authorized access only</p>
        </div>

        <form
          method="POST"
          action="/api/drafts/login"
