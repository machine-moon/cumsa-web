...
export default function SecretMenu({
  ...
  return (
    <div
      ...
      className="absolute z-50 bottom-10 left-0 w-72 max-h-[70vh] overflow-y-auto rounded-lg border border-black/10 bg-white shadow-lg p-3 flex flex-col gap-3 text-sm animate-fadeIn"
    >
      {SECRET_SECTIONS.map((section, idx) =>
        section.title === "" ? (
          <SectionBlank key={"section-blank-" + idx} section={section} />
        ) : (
...
function SectionBlank({ section }: { section: SecretSection }) {
  return (
...
function SectionDropdown({ section }: { section: SecretSection }) {
  return (
    <details className="group border rounded mb-0">
      <summary className="list-none cursor-pointer select-none px-2 py-1 flex items-center justify-between font-semibold text-[var(--navy)] text-xs uppercase tracking-wide opacity-80">
